...
def resim_analiz_et(resim_yolu):
    ...
    # BGR formatından RGB formatına çevir (Tkinter düzgün göstersin diye)
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    # --- MODEL İÇİN ÖN İŞLEME (PREPROCESSING) ---
    img_resized = cv2.resize(frame, (256, 256))
    img_array = img_resized.astype("float32")  # Normalizasyon yok, model kendi içinde yapıyor
    ...
    # Güven oranına göre yazı rengini ayarla (Dengeli tahmin koruması)
    if confidence > 65:
        lbl_sonuc.config(fg="#2ecc71")  # Yeşil
    else:
        lbl_sonuc.config(fg="#e67e22")  # Turuncu (Emin değil)

    # Seçilen resmi arayüzde göstermek için boyutlandırıp hazırla
...
# --- 3. DOSYA SEÇME PENCERESİ ---
def dosya_sec():
    # Kullanıcıya bilgisayarındaki dosyaları açtırır
    dosya_yolu = filedialog.askopenfilename(
        filetypes=[("Resim Dosyaları", "*.jpg *.jpeg *.png")]
    )

    if dosya_yolu:
        # Eğer bir dosya seçildiyse analiz fonksiyonuna gönder
        resim_analiz_et(dosya_yolu)


# --- 4. TKINTER ARAYÜZ TASARIMI ---
pencere = tk.Tk()
...
# Resmin Gösterileceği Alan (Boş çerçeve)
lbl_resim = tk.Label(pencere, bg="#f5f6fa")
lbl_resim.pack(pady=15)

# Sonuçların Yazılacağı Alan
lbl_sonuc = tk.Label(
    pencere,
